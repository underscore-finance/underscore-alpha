...
def test_basic_yield_tracking(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault, price_sheets):
    ...
    # test yield tracking in config
    assert ai_wallet_config.isVaultToken(vaultToken)
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited
...
def test_yield_tracking_on_exit(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived  # Should not change
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived - vaultTokenAmountBurned
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited - assetAmountReceived


def test_yield_tracking_on_entry(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert not ai_wallet_config.isVaultToken(non_vault_token)
    alpha_token.transfer(ai_wallet, entry_amount, sender=alpha_token_whale)
    ai_wallet.transferFunds(owner, entry_amount, non_vault_token, sender=owner)
    assert not ai_wallet_config.isVaultToken(non_vault_token)  # Should remain false


def test_yield_tracking_on_withdrawal(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived - vaultTokenAmountBurned
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited - assetAmountReceived_one
...
def test_yield_tracking_exact_amounts(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
...
def test_yield_tracking_invalid_lego(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived - entry_amount
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited - (assetAmountDeposited * entry_amount // vaultTokenAmountReceived)


def test_extra_vault_tokens(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
...
def test_extra_vault_tokens_withdrawal_within_tracked_amount(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived  # Should not change since actual balance >= tracked balance
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited  # Should not change since actual balance >= tracked balance
    assert alpha_token_erc4626_vault.balanceOf(ai_wallet) == vaultTokenAmountReceived - vaultTokenAmountBurned + extra_amount


def test_extra_vault_tokens_withdrawal_outside_tracked_amount(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    # verify tracking is correct
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived // 2  # Should reduce by half since actual balance < tracked balance
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited // 2  # Should reduce proportionally
    assert alpha_token_erc4626_vault.balanceOf(ai_wallet) == vaultTokenAmountReceived // 2


def test_extra_vault_tokens_transfer(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == expected_vault_tokens
    assert ai_wallet_config.depositedAmounts(vaultToken) == expected_deposited
    assert alpha_token_erc4626_vault.balanceOf(ai_wallet) == expected_vault_tokens


def test_extra_vault_tokens_mixed_operations(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == vaultTokenAmountReceived - transfer_amount  # Should be reduced by transfer amount
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited - (assetAmountDeposited * transfer_amount // vaultTokenAmountReceived)  # Should be reduced proportionally
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == (vaultTokenAmountReceived - transfer_amount) + new_vaultTokenAmountReceived  # Reduced original (75) + new deposit (75)
    assert ai_wallet_config.depositedAmounts(vaultToken) == (assetAmountDeposited - (assetAmountDeposited * transfer_amount // vaultTokenAmountReceived)) + new_assetAmountDeposited  # Reduced original + new deposit
...
def test_yield_tracking_edge_cases(mock_lego_alpha, alpha_token, alpha_token_whale, ai_wallet, ai_wallet_config, owner, alpha_token_erc4626_vault):
    lego_id = mock_lego_alpha.legoId()
    deposit_amount = 100 * EIGHTEEN_DECIMALS
    ...
    assert not ai_wallet_config.isVaultToken(alpha_token_erc4626_vault)
    ...
    assert ai_wallet_config.isVaultToken(vaultToken)
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == initial_tracked_amount
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == initial_tracked_amount + extra_vault_tokens
    assert ai_wallet_config.depositedAmounts(vaultToken) == assetAmountDeposited + extra_asset_deposited
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == expected_remaining_tracked
    ...
    previous_tracked_amount = ai_wallet_config.vaultTokenAmounts(mock_vault_token)
    ...
    assert ai_wallet_config.vaultTokenAmounts(mock_vault_token) == previous_tracked_amount - 25 * EIGHTEEN_DECIMALS
    ...
    previous_tracked = ai_wallet_config.vaultTokenAmounts(vaultToken)
    ...
    assert ai_wallet_config.vaultTokenAmounts(vaultToken) == previous_tracked - small_amount